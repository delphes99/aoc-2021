fun main() {
    ...
    println("part 2 :")
    println(Day4Part2.calculate(readLinesOf("day4.txt")))
}
...
class Day4Part2 {
    companion object {
        fun calculate(entry: List<String>): Int {
            ...
            var lastWinningBoards: Pair<List<Int>, List<Board>>? = null
            numberToCall.forEach { callNumber ->
                calledNumber.add(callNumber)

                boards
                    .filter { it.isWinning(calledNumber) }
                    .filter { !winningBoards.contains(it) }
                    .takeIf { it.isNotEmpty() }
                    ?.also { newWinningBoards ->
                        winningBoards.addAll(newWinningBoards)
                        lastWinningBoards = calledNumber.toList() to newWinningBoards
                    }
            }

            return lastWinningBoards?.let { (winningCallNumbers, winningBoard) -> winningBoard.first().scoreFor(winningCallNumbers) } ?: error("no winning board")
        }
    }
